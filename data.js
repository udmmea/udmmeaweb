function createTeamLogoSvg(teamName = 'Team') {
  const safeName = String(teamName || 'Team').trim();
  const initials = safeName
    ...
    .join('');
  const colors = ['#5B8CFF', '#FF6B6B', '#2EC4B6', '#F4A261', '#8B5CF6', '#22C55E', '#F59E0B', '#EC4899'];
  const hash = Array.from(safeName).reduce((acc, char) => acc + char.charCodeAt(0), 0);
  const bg = colors[hash % colors.length];
  const accent = colors[(hash + 3) % colors.length];
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
    <rect width="160" height="160" rx="36" fill="${bg}"/>
    <circle cx="128" cy="36" r="20" fill="${accent}" opacity="0.85"/>
    <circle cx="40" cy="128" r="24" fill="#ffffff" opacity="0.18"/>
    <text x="80" y="92" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="44" font-weight="700" fill="#ffffff">${initials || 'T'}</text>
  </svg>`;
...
function normalizeTeams(teamList) {
  return (Array.isArray(teamList) ? teamList : []).map(team => ({
    ...
    logo: team.logo || createTeamLogoSvg(team.name)
  }));
}
  ...
  function loadState(){
    try{
      ...
      if(ts){
        ...
        if(Array.isArray(parsed) && parsed.length) teams = normalizeTeams(parsed);
      }
      if(ms){
  ...
  slotContainer.innerHTML = slots.map(slot =>
    `<a class="player-item" href="${slot.telegram}" target="_blank" rel="noreferrer noopener">
      <div style="display:flex;align-items:center;gap:10px;">
        <img src="${slot.logo || ''}" alt="${slot.name}" style="width:28px;height:28px;border-radius:8px;object-fit:cover;">
        <div>
